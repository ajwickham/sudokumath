...
export default class SudArray {
  constructor(grid,solution) {
    this.grid = grid;
    this.solution = solution;
  }

  retryEarlierNumbers (problemPosition) {
    let x;
    for(let k = 1; k<9; k++){
  ...
  checkColumn(position,startNumber) {
    for (let j = startNumber; j<11; j++){  //tries startNumber-9 in turn to find the lowest missing number
      let count = 0;
  ...
  checkSquare(position,startNumber) {
    let count = 0;
  ...
  checkManualRow(position,number) {
    if (number != 0){
      let count = 0;
      for (let i=(parseInt(position/9))*9; i<((parseInt(position/9))*9)+9; i++) {  
  ...
  checkManualColumn(position,number) {
    if (number != 0){
      let count = 0;
      for (let i=(position%9); i<81; i=i+9) {  
  ...
  checkManualSquare(position,number) {
    if (number != 0){
      let count = 0;
      let col = Math.trunc((position%9)/3);
      let ro = Math.trunc(Math.trunc(position/9)/3);
